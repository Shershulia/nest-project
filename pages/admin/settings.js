...
const SettingsAdminPage = ({ isAdmin, mainPictures: loaded }) => {
  ...
  const changeMainPictures = (ev) => {
    ...
    if (JSON.stringify(loaded.images) === JSON.stringify(mainPictures)) {
      console.log("GHEello");
      Swal.fire("Alert!", `You made no changes in images`, "question");
    } else {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          await axios
            .put("/api/admin/settings", { ...loaded, images: mainPictures })
            .then((res) => {
              Swal.fire({
  ...
  return isAdmin ? (
    <div className="h-full flex">
      <AdminLayout backgroundColor={"customPurple"}>
        <div className={"flex flex-col items-center"}>
          ...
          <button
            className={
              "bg-customDarkPurple hover:bg-customLightPurple transition-all w-fit text-lg font-bold p-2 rounded-md border-black mx-4"
            }
            onClick={(event) => changeMainPictures(event)}
          >
            Save changes
          </button>
        </div>
