...
const Switcher = ({ title,value, setValue , className = "" }) => {
    const changeValue = () => {
    ...
    return (
        <label className={`relative inline-flex items-center cursor-pointer ${className}`}>
            <input type="checkbox" value={value} onChange={changeValue} className="sr-only peer"/>
                <div
                ...
                <span className="ms-3 md:text-sm  font-medium text-gray-900 dark:text-gray-300 truncate overflow-hidden text-xs">{title}</span>
        </label>
    )
};
